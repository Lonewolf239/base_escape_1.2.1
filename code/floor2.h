...
void floor2(int fl2, int fr2) { //есть ключ
	...
	case 1:
		...
		break;
	case 2:
		cout << "___________________________________________\You have reached the door\nOpen 1; Leave 2" << endl;
		cin >> bed2;
		if (bed2 == 1)
			bedroom2(1);
		if (bed2 == 2)
...
void floor3(int fl3, int fr3) { //открыта спальня
	...
	if (fl3 == 1) {
		cout << "___________________________________________\nYou are standing next to the stairs\n(It looks like a door is open somewhere)\nStudy 1\nBedroom 2\nRecreation room 3\nAttic 4" << endl;
		cin >> fr3;
	}
	switch (fr3) {
	case 1:
		cout << "___________________________________________\nYou've reached the door... well, now it's open\nEnter 1; Leave 2" << endl;
		cin >> cab3;
		if (cab3 == 1)
			cabinet3(1);            //Кабинет
		if (cab3 == 2)
